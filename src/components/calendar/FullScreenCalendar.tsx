...
export function FullScreenCalendar({ events, onEventClick, onDateClick }: FullScreenCalendarProps) {
  ...
  // Convert CalendarEvent[] to CalendarData[] format
  const calendarData: CalendarData[] = React.useMemo(() => {
    const daysWithEvents = new Map<string, CalendarData>()
    ...
    events.forEach(event => {
      ...
      if (!daysWithEvents.has(dateKey)) {
        daysWithEvents.set(dateKey, {
          day: eventDate,
          events: []
        })
      }
      
      const dayData = daysWithEvents.get(dateKey)!
      dayData.events.push({
        id: event.id,
        name: event.title,
    ...
    return Array.from(daysWithEvents.values())
  }, [events])
    ...
    <div className="flex flex-1 flex-col">
      ...
      <div className="flex flex-col space-y-4 p-4 md:flex-row md:items-center md:justify-between md:space-y-0 lg:flex-none">
        ...
        <div className="flex flex-col items-center gap-4 md:flex-row md:gap-6">
          ...
          <Button onClick={() => onDateClick && onDateClick(selectedDay)} className="w-full gap-2 md:w-auto">
